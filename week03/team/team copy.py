...
class Board():
    ...
    def find_word(self, word):
        ...
        for row in range(self.size):
            for col in range(self.size):
                for d in range(0, 8):
                    if self._word_at_this_location(row, col, d, word)[0]:
                        return self._word_at_this_location(row, col, d, word)[1]
        return False
...
def main():
    ...
    # with mp.Pool(12) as hottub:
    #     output = hottub.map(board.find_word, words)
    #     board.highlight_positions(output)
